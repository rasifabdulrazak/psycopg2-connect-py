...
def data_migration_expense(file_path: str, table_name: str):
    ...
    with open(file_path, "r") as file:
        ...
        for i in data:
            date_obj = datetime.strptime(i.get('DATE'), "%d/%m/%Y")
            expense_amount_str = i.get('expense_amount', '0').replace('₹', '').replace(',', '').strip()
            expense_amount = decimal.Decimal(expense_amount_str)
            i['expense_amount'] = expense_amount
            i.pop('DATE', None)
            i.pop('uploaded_by_id',None)
            ...
            if ids:
                i['property_id_id'] = ids[0]
                i['created_at'] = date_obj
                i['uploaded_by_id'] = 76
                placeholders = ", ".join(["%s"] * len(i))
                print(columns,"ppp")
                print(list(values))
                select_query =  f"""INSERT INTO {table_name} ({columns},property_id_id,created_at,uploaded_by_id) VALUES ({placeholders})"""
                print(select_query,";;;")
